# Application Q&A: OpenAI Product Manager API Agents
...
OpenAI is where the most consequential agentic primitives in software are getting built. The API team is the layer that decides how millions of developers actually use those primitives. Being the PM on that team means I get to shape the developer experience for every agent that gets shipped on top of OpenAI's models.

I have been shipping the customer-side version of this work at ESC for six months. Maya AI runs production traffic through AWS Bedrock across Claude Opus, Sonnet, and Nova for a 200K-resident utility. The routing logic, confidence scoring, and operator-takeover UX are exactly the primitives that the API Agents team owns at platform scale.

---

## Why this role?

Four lines from the JD describe how I already work:

1. **Deeply understand problems faced by agent builders.** I am one. Maya AI is my agent. I built the routing layer, the eval framework, and the operator surface. I know where the rough edges are because I have hit them.
2. **Define strategic priorities and roadmap for agentic infrastructure.** ESC: built the AI roadmap from scratch as first hire. CloudApproach: ran roadmap for two ventures simultaneously.
3. **Partner with research and engineering teams at a technical level.** Daily Claude Code operator. I write the FastAPI handlers, design the Bedrock prompts, and ship the React UI myself.
4. **Deliver quickly while maintaining a high bar for product quality.** Maya AI: 2.5 months from no playbook to live at 200K residents.
